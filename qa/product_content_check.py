...
check('Hotfix8 verified uninstall cleanup', 'BRIDGEX_INSTALL_MARKER' in installer and 'RMDir /r /REBOOTOK "$INSTDIR"' in installer and 'BridgeX-CloseInstalled.ps1' in installer)
...
check('Close helper is not persisted in Program Files', '"$INSTDIR\\BridgeX-CloseInstalled.ps1"' not in installer and '"$TEMP\\BridgeX-CloseInstalled.ps1"' in installer)
