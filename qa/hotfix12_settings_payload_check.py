...
check('installer has no PowerShell execution-policy bypass', 'powershell.exe' not in installer.lower() and 'executionpolicy bypass' not in installer.lower())
check('uninstaller uses NSIS reboot fallback for locked files', 'SetOutPath "$TEMP"' in installer and 'RMDir /r /REBOOTOK "$INSTDIR"' in installer)
